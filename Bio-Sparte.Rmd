...
# Create a basic bar
ggplot(org_notorg, aes(x = "", y = n_produkte, fill = organic)) + geom_bar(stat = "identity", width = 1) +
...
```{r}
Verhalt_Bio_bought <- full_join(t_products, t_orders_product, "product_id") %>%
...
# Create a basic bar
ggplot(Verhalt_Bio_bought, aes(x = "", y = n, fill = organic)) + geom_bar(stat = "identity", width = 1) +
  # Convert to pie (polar coordinates) and add labels
  coord_polar("y", start = 0) + 
...
### Beliebheit der Bio-Produkte in den Departments/aisles

```{r}
Product_sold_dept_beliebheit <- full_join(t_products, t_orders_product, "product_id") %>%
  ...
  mutate(n = as.integer(n)) %>%
  mutate(nn = lag(n)) %>%
  mutate(sold_organic = n, sold_notorganic = nn) %>%
  mutate(tot_sold_dep = sum(sold_organic)) %>%
  select(-c(n,nn, organic)) %>%
  filter(!is.na(sold_notorganic)) %>%
  mutate(sold_bio_prozent = round(100*sold_organic/tot_sold_dep, digits = 2)) %>%
  arrange(desc(sold_bio_prozent))

#singlton pattern

Product_sold_dept_beliebheit

ggplot(Product_sold_dept_beliebheit, aes(x = reorder(department, sold_bio_prozent), y = sold_bio_prozent)) +
  labs(x = "department", y = "sold_bio_prozent",  title = "Prozentanzahl gekaufte Bio-Produkte per Department") +
  geom_col(fill="orange") +
  geom_text(aes(label = paste0(sold_bio_prozent, "%")), position = position_stack(vjust = 0.5)) + 
  coord_flip()
```
...
```{r}
Product_sold_aisle_beliebheit <- full_join(t_products, t_orders_product, "product_id") %>%
  ...
  #mutate(sold_organic = 1, sold_notorganic = 0) %>%
  #mutate(tot_sold_aisle = sum(sold_organic)) %>%
  #select(-c(0,1, organic)) %>%
  #filter(!is.na(sold_notorganic)) %>%
  #mutate(sold_bio_prozent = round(100*sold_organic/tot_sold_aisle, digits = 2)) %>%
  #arrange(desc(aisle))

#singlton pattern

Product_sold_aisle_beliebheit

ggplot(Product_sold_aisle_beliebheit, aes(x = reorder(aisle, sold_bio_prozent), y = sold_bio_prozent)) +
  labs(x = "department", y = "sold_bio_prozent",  title = "Prozentanzahl gekaufte Bio-Produkte per Department") +
  geom_col(fill="orange") +
  geom_text(aes(label = paste0(sold_bio_prozent, "%")), position = position_stack(vjust = 0.5)) + 
  coord_flip()
...
```{r}
t_orders_FirstBuy
t_orders_FirstBuy %>% inner_join(t_orders_product) %>%
