...
```{r}
proportion_bio_bought <- full_join(t_products, t_orders_product, "product_id") %>%
...
# Create a basic bar
ggplot(proportion_bio_bought, aes(x = "", y = n, fill = organic)) + geom_bar(stat = "identity", width = 1) +
  # Convert to pie (polar coordinates) and add labels
  coord_polar("y", start = 0) + 
...
proportion_dep <- full_join(t_products, t_departments, "department_id") %>%
  ...
  mutate(prod_bio_percent = round(100*sold_organic/tot_prod_dep, digits = 2)) %>%
  arrange(desc(prod_bio_percent))

proportion_dep

ggplot(proportion_dep) +
