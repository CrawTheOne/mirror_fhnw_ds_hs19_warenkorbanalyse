...
```  
Aus dem Diagramm ist zu entnehmen, dass Bestellungen 24 Stunden lang pro Tag eingehen. Ab Stunde 5 ist ein Starker Anstieg der eingehenden Produkte bemerkbar bis ca. Stunde 10. Von Stunde 17 bis 24 reduzieren sich die Bestellungen wieder 
...
order_product_ratio <- orders_per_hour_per_day %>%
  ...
  mutate(order_product_ratio  = total_product_count/total_order_count, workload_orders = total_order_count/max_o, workload_products = total_product_count/max_p, mean_workload_orders = total_order_count/mean_o, mean_workload_products = total_product_count/mean_p)
...
ggplot(data = order_product_ratio) + 
  ...
  ggtitle("Average nr of products per hour as heatmap")+
  xlab("Order hour") + ylab("Order day") +
...
ggplot() + 
  ...
  ggtitle("Relations")  +
  xlab("Open From XY %") + ylab("Relative Percentage") +
  labs(color='Weekday') +
...
Dieser Graf zeigt das verhältniss zwischen den einzelnen Öffnungsschwellenwerte und dem Delta aus Öffnungsdauer und Produktverkäufen zum dauergeöffneten Betrieb. Aus dem Diagramm ist erkennbar, dass eine Öffnungsschwelle von 15% die grösste Zeitersparrnis im Verhältnis zu den verkauften Produkten mit sich führt.
...
Damit die Ladenöffnungszeiten einigermassen konstannt sind, empfehlen wir an Tag 4 und 6 je noch eine Stunde früher zu schliessen. Daturch ergeben sich folgende Öffnungszeiten:
...
Die Auslastung beträgt pro Stunde durchschnittlich **`r round(avg_workload, digits = 1)` Prozent** (Gemessen an der Stunde mit der grössten Auslastung = 100%.), was einer Erhöhung von durchschnittlich **`r round((avg_workload/avg_workload_0), digits = 1)*100-100` Prozent** pro Stunde entspricht.

Bei diesen Öffnungszeiten werden **`r round(P_Percent_loss_final, digits = 1)` Prozent** der Verkauften Produkte im Datensatz nicht abgedeckt. 
