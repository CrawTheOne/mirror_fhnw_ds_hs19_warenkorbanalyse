...
Für die Warenkorbanalyse benötigen wir die Transaktionen als Key und kommasepariert alle Produkte, welche zu dieser Transaktion gehören. Dies erreichen wir mit Query:
...
```{r}
rules <- apriori(transactions, parameter = list(supp = 0.0025, conf = 0.5, target="rules"))
...
```{r}
orders_to_departments <- orders_to_products %>%
  select(orders_id, department) 

total_orders <- orders_to_departments %>%
  count()

orders_to_departments %>%
  group_by(department) %>%
  summarise(n = n()) %>%
  mutate(support = as.numeric(n)/total_products_bought) %>%
  arrange(desc(support))
...
```{r}
rules_department <- apriori(transactions_department, parameter = list(supp = 0.3, conf = 0.5, target="rules"))
rules_department <- rules_department[!is.redundant(rules_department)]
rules_tib <- 
