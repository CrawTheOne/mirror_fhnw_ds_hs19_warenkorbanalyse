...
customer_behavior %>%
  ...
  ggplot(aes(x=total_nr_of_orders, y=fraction_mean_bio)) +
    ...
    geom_smooth() +
    labs(x = "Total number of orders", y = "Ratio of bio products", title = NULL)
```
...
customer_behavior %>%
  ...
  ggplot(aes(x=total_product_count, y=fraction_mean_bio)) +
    geom_point(alpha = 0.01) +
    geom_smooth() +
    labs(x = "total products bought", y = "Ratio of bio products", title = NULL)
```
...
In allen drei Graphen kann man sehen, dass der Bioanteil bei grösserer "Nutzung" des Stores ansteigt.

## Welche Bioprodukte werden gern zusammen gekauft
...
transactions <- as(bio_baskets$items, "transactions")
