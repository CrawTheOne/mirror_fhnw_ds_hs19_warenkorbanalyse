...
```{r}
customer_behavior %>%
  mutate(fraction_mean_bio = round(fraction_mean_bio, digits = 1)) %>%
  group_by(total_nr_of_orders, fraction_mean_bio) %>%
  summarise(weight = sum(fraction_mean_bio)) %>%
  ggplot(aes(x=total_nr_of_orders, y=fraction_mean_bio, fill=weight)) +
    geom_tile() +
    scale_fill_gradientn(colors = c("blue", "lightblue", "green", "red"), values = c(0,0.1, 0.3, 1)) +
    labs(x = "Total number of orders", y = "Ø ratio of bio products", fill = "frequency", title = NULL)
```
